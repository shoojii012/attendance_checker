...
def main(mac_address_list = '/home/attendance_checker/mac_address_list.csv'):
    ...
    for i in range(len(mac_address)):
        if output.find(mac_address[i][1])!=-1 or output.find(mac_address[i][2])!=-1 or output.find(mac_address[i][3])!=-1 or output.find(mac_address[i][4])!=-1:
...
def user_exit(name,now_time):
    ...
    for i in range(len(attendance)):
        if attendance[i][0]==name:
            entry_time = attendance[i][1]
            with open('log.csv','a') as f:
                writer = csv.writer(f)
                writer.writerow([attendance[i][0], attendance[i][1],attendance_data[0][0]])
            del attendance[i]
